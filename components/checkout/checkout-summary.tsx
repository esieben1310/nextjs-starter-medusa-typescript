...
const CheckoutSummary: FunctionComponent<Props> = ({cart}) => {
  ...
  return cart ? (
      <div className={`${styles.container} ${orderSummary ? styles.active : ""}`}>
        ...
        <div className={styles.breakdown}>
          ...
          <span>
          {cart.region
              ? formatPrice(cart.subtotal || 0, cart.region.currency_code)
              : 0}
        </span>
        </div>
        <div className={styles.breakdown}>
          ...
          <span>
          {cart.region
              ? formatPrice(cart.shipping_total || 0, cart.region.currency_code)
              : 0}
        </span>
        </div>
        <div className={styles.total}>
          <p>Total</p>
          <span>
          {cart.region ? formatPrice(cart.total || 0, cart.region.currency_code) : 0}
        </span>
        </div>
      </div>
  ) : (
